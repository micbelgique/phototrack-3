...
namespace Trasys.Dev.Tools.Logger
{
    ...
    /// </summary>
    /// <remarks>
    /// Code from http://bernhardelbl.wordpress.com/2013/01/12/how-to-write-log-files-in-windows-store-apps/
    /// </remarks>
    internal class StorageFileEventListener : EventListener
    {
        private StorageFile _storageFile = null;                                    // Storage file to be used to write logs
        private string _name;                                                       // Name of the current event listener
        private string _format = "{0:yyyy-MM-dd HH\\:mm\\:ss\\:ffff}\t{1}\t{2}";    // The format to be used by logging.
        private volatile List<string> _linesCache = new List<string>();             // Contains the local cache of the lines
        private volatile int _linesToProcess;                                       // Contains the number of lines that must be written to the log file
        private object _syncRoot = new object();                                    // Contains the sync root for the lines cache
        private ThreadPoolTimer _timer;                                             // Contains a delay timer
        ...
        {
            var newFormatedLine = string.Format(_format, DateTime.Now, eventData.Level.ToString()[0], eventData.Payload[0]);
        ...
        void CheckLogFileSize()
        {
            try
            {
                ...
                Wait(bufferAction);
                // when the log file reaches 10 mb file size, copy the log to bak file and empty the log file.
                int mb = 10 * 1024 * 1000;
        ...
        void Wait(IAsyncOperation<global::Windows.Storage.Streams.IBuffer> bufferAction)
        {
